...
import { IconTarget, IconFileText, IconFilter, IconFolder, IconChevronDown, IconChevronRight, IconUstek, IconEye, IconSearch, IconTrophy, IconClock, IconGlobe, IconUser, IconCalendar } from '../../components/Icons';
...
const TAHAPAN_ORDER = ['Evaluasi PQ', 'Upload PQ', 'Pembuktian', 'Penyusunan Ustek', 'Upload Ustek'];
...
const getTahapanRank = (key: string) => {
  ...
  const availableTahapanOptions = useMemo(() => {
    ...
    if (filterStatus === 'ALL') {
      tahapanList = Array.from(new Set(biddingOnly.map((p) => p.tahapan || 'Evaluasi PQ')));
    } else {
      const filteredByStatus = biddingOnly.filter((p) => (p.status_project || 'Ongoing') === filterStatus);
      tahapanList = Array.from(new Set(filteredByStatus.map((p) => p.tahapan || 'Evaluasi PQ')));
    }
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      tahapan: isPL ? 'Penunjukan Langsung' : tahapan,
      status_project: isPL ? 'Menang' : statusProject,
      deadline_pengumuman: isPL ? null : (deadlinePengumuman || null),
      peringkat: isPL ? 'PL' : (peringkat || '-'),
      prioritas: prioritas,
  ...
  const filteredBiddingProjects = useMemo(() => {
    ...
    return rawBiddingProjects.filter((p) => {
      ...
      const matchTahapan = filterTahapan === 'ALL' || (p.tahapan || 'Evaluasi PQ') === filterTahapan;
      const matchQuery = !q || (
        (p.nama_pekerjaan || '').toLowerCase().includes(q) ||
        (p.pemberi_kerja || '').toLowerCase().includes(q) ||
        (p.satuan_kerja || '').toLowerCase().includes(q) ||
  ...
  const groupedBiddingProjects = useMemo(() => {
    ...
    if (groupBy === 'both') {
      ...
      filteredBiddingProjects.forEach((p) => {
        ...
        const tKey = `Tahapan: ${p.tahapan || 'Evaluasi PQ'}`;
    ...
    filteredBiddingProjects.forEach((p) => {
      const key = groupBy === 'status' ? `Status: ${p.status_project || 'Ongoing'}` : `Tahapan: ${p.tahapan || 'Evaluasi PQ'}`;
  ...
  // Calendar events for Bidding Deadlines
  ...
  return (
    <div>
      <div className="page-header">
        <h1 className="page-title">Daftar Bidding & Penunjukan Langsung (IR)</h1>
        <p className="page-desc">Workspace Marketing untuk mengelola Bidding dan Penunjukan Langsung.</p>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))' }}>
            <div className="stat-card">
              <span className="stat-label">Total Pekerjaan Bidding</span>
            ...
            <div className="stat-card">
              <span className="stat-label">Bidding Ongoing</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
            <div className="glass-card">
              <h3 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <IconTarget size={20} color="#6366f1" /> Breakdown Prioritas Bidding
              </h3>
            ...
            <div className="glass-card">
              ...
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '1rem' }}>
                Jumlah Karyawan terdaftar: <strong>{internalUsers.length} Orang</strong>. Mengelola PIC untuk penugasan Bidding & Ustek.
              </p>
          ...
          <div>
            <CalendarWidget
              title="Jadwal Deadline Pengumuman Bidding"
              type="bidding"
        ...
        <>
          ...
          <div className="glass-card" style={{ marginBottom: '2rem' }}>
            ...
            <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 1fr 1.2fr 1fr', gap: '1rem' }}>
                <div className="input-group">
                                        ...
                                        <tr key={p.id}>
                                          ...
                                          <td>
                                            <SearchableSelect
                                              ...
                                              value={p.tahapan || 'Upload PQ'}
                                              onChange={(val) => handleUpdate(p.id, { tahapan: val })}
                                              options={['Upload PQ', 'Evaluasi PQ', 'Pembuktian', 'Penyusunan Ustek', 'Upload Ustek']}
                                            />
                                          </td>
                                          <td>
